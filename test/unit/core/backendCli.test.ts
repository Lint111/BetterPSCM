...
describe('CliBackend', () => {
	...
	describe('getReviewComments', () => {
		it('throws NotSupportedError (review comments require REST API)', async () => {
			await expect(backend.getReviewComments(43381)).rejects.toThrow('not supported');
		});
	});

	describe('getReviewers', () => {
		it('throws NotSupportedError (reviewers require REST API)', async () => {
			await expect(backend.getReviewers(43381)).rejects.toThrow('not supported');
		});
	});
